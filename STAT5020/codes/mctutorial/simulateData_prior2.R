...
for (t in 1:iter) {
  iterpath = paste0(getwd(),"/reppr", t)
  dir.create(iterpath, showWarnings = FALSE, recursive = TRUE)
  
  Y = as.matrix(read.table(paste(datapath, "Y-", t, ".txt", sep = "")))
  BD = read.table(paste(datapath, "BD-", t, ".txt", sep = ""))$x
  BC = read.table(paste(datapath, "BC-", t, ".txt", sep = ""))$x
  
  data = list(N = 500, zero = c(0, 0), d = BD, c = BC, R = R, y = Y)
  
  model = bugs(data, inits, parameters, model.file = paste0(getwd(),"/../model_prior2.txt"), 
               n.chains = 3, n.iter = 3000, n.burnin = 1000, 
               n.thin = 1, bugs.directory = winBUGS.path,
               working.directory = iterpath, debug = TRUE)
...
# load true values of parameters

load(paste0(getwd(),"/trueparam.RData"))
...
save(resultlst, file = paste0(getwd(),'/modelprior2-', timestamp, ".RData"))
